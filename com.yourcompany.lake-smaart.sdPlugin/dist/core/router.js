...
class Router {
    ...
    contextMap = new Map();
    sdClient;
    constructor(sdClient) {
        this.sdClient = sdClient;
    }
    registerAction(uuid, action) {
    ...
    route(event) {
        ...
        if (event.event === 'willAppear') {
            ...
            if (actionFactory) {
                ...
                if (actionFactory.onWillAppear)
                    actionFactory.onWillAppear(e);
            }
            return;
        }
        if (event.event === 'willDisappear') {
            ...
            if (action) {
                if (action.onWillDisappear)
                    action.onWillDisappear(e);
        ...
        switch (event.event) {
            ...
            case 'keyUp':
                ...
                break;
            case 'dialRotate':
                if (action.onDialRotate)
                    action.onDialRotate(event);
                break;
            case 'dialDown':
                if (action.onDialDown)
                    action.onDialDown(event);
                break;
            case 'dialUp':
                if (action.onDialUp)
                    action.onDialUp(event);
                break;
            case 'didReceiveSettings':
                if (action.onDidReceiveSettings)
