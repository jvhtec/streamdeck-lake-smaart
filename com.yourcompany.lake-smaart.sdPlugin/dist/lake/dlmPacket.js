...
exports.ACK_NOTMASTER = exports.ACK_SUCCESS = void 0;
exports.encodeDlmMsg = encodeDlmMsg;
exports.decodeAck = decodeAck;
exports.parseResponse = parseResponse;
exports.ACK_SUCCESS = 0;
exports.ACK_NOTMASTER = 1;
function encodeDlmMsg(commandString, msgId, destId = '0', // Default or specific
destClass = '0', requireAck = true) {
    // NOTES:
    ...
    offset += 2; // Version/Type
    buf.writeUInt16BE(requireAck ? 1 : 0, offset);
    offset += 2; // Flags
    // Message ID
    buf.writeUInt32BE(msgId, offset);
    offset += 4;
    // Length (Payload length)
    buf.writeUInt32BE(payloadBuffer.length, offset);
    offset += 4;
    // Copy payload
    payloadBuffer.copy(buf, offset);
    return buf;
}
function decodeAck(packet) {
    // Trying to parse an ACK packet.
    // Assuming ACK packet mirrors the header structure.
    if (packet.length < 12)
        return null;
    let offset = 4; // Skip Type/Flags
    const msgId = packet.readUInt32BE(offset);
    offset += 4;
    ...
    return { msgId, status: exports.ACK_SUCCESS };
}
